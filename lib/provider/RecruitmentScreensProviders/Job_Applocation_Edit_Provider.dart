...
class JobApplicationEditProvider extends ChangeNotifier {
  ...
  // Current job application data
  JobApplicationModel? _currentJobApplication;
  JobApplicationModel? get currentJobApplication => _currentJobApplication;
  ...
  // Status and assignment variables
  ...
  void setUpdatingStatus(bool updating) {
  ...
  /// Dropdown data
  final List<String> _primaryLocation = ["Aathur", "Aasam", "Nagapattinam", "Bengaluru - Hebbal"];
  ...
  final List<String> _jobStatus = ["Unread", "Call for Interview", "Selected", "Rejected"];
  ...
  /// Load specific job application by ID
  Future<void> loadJobApplication(String jobId) async {
    _isLoading = true;
    notifyListeners();
    ...
    try {
      ...
      // Find job application from sample data or API
      final jobApp = _allJobApplication.firstWhere(
            (app) => app.jobId == jobId,
        orElse: () => JobApplicationModel(
          jobId: "JA18184",
          name: "SATHISH KUMAR R",
          phone: "91******47",
          jobTitle: "Software Developer",
          primaryLocation: "Nagapattinam",
          uploadedBy: "https://example.com/recruiter.jpg",
      ...
      _currentJobApplication = jobApp;
      _selectedStatus = 'Unread'; // Set default status
    } catch (e) {
      debugPrint("Error loading job application: $e");
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  Future<bool> updateJobApplicationStatus(String jobId, String status) async {
    try {
  ...
  /// Assign job application to staff
  Future<bool> assignJobApplication(String jobId, String assignee) async {
    try {
      setAssigning(true);
      ...
      debugPrint("Job assigned to: $assignee");
      return true;
    } catch (e) {
  ...
  List<Map<String, String>> getJobApplicationHistory() {
    return [
      {
        'date': '26-09-2025 10:30 AM',
        'action': 'Application Created',
        'user': 'System',
        'status': 'Unread'
      },
      {
        'date': '26-09-2025 02:15 PM',
        'action': 'Status Updated',
        'user': 'Admin User',
        'status': 'Under Review'
      },
      {
        'date': '26-09-2025 04:45 PM',
